...
export default function MonzoAccountSection({
  ...
  return (
    <div className="border border-purple-600 bg-gradient-to-r from-purple-900/20 to-blue-900/20 p-4 sm:p-6 rounded-lg">
      ...
      <MonzoStatusSection currentUser={currentUser} data={data} />
        ...
        <div className="space-y-4">
          {monzoTransactionAccounts.map((account) => (
            ...
            >
              ...
              {account.balance ? (
                <MonzoAccountsBalanceSection account={account} />
              ) : (
      ...
      {monzoSavingsAccounts.length > 0 && (
        <MonzoSavingsAccountsSection
          monzoSavingsAccounts={monzoSavingsAccounts}
        />
      )}
    </div>
  );
...
// ----------------------------------
interface MonzoStatusSectionProps {
  currentUser: User | null;
  data: BankingData;
}

function MonzoStatusSection({ currentUser, data }: MonzoStatusSectionProps) {
  ...
  //   TODO: this logic feels wrong, should it check if there are automations active
...
// ----------------------------------
interface MonzoAccountsBalanceSectionProps {
  account: AccountData & {
    balance?: AccountBalance;
  };
}

function MonzoAccountsBalanceSection({
  account,
}: MonzoAccountsBalanceSectionProps) {
  return (
      ...
      {account.type === 'uk_monzo_flex' ? (
        ...
        <>
          ...
          {account.balance?.available !== undefined &&
            account.balance.available !==
              (account.balance.current || account.balance.balance) && (
              <div className="flex justify-between items-center">
        ...
        <>
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-semibold text-white">
              {formatCurrency(
                account.balance?.current || account.balance?.balance || 0,
                account.balance?.currency || account.currency,
                true // isMonzo
              )}
            </span>
...
// ----------------------------------
interface MonzoSavingsAccountsSectionProps {
  monzoSavingsAccounts: (AccountData & {
    balance?: AccountBalance;
  })[];
}

function MonzoSavingsAccountsSection({
  monzoSavingsAccounts,
}: MonzoSavingsAccountsSectionProps) {
  const [savingsExpanded, setSavingsExpanded] = useState<boolean>(false);

  const toggleSavingsExpanded = () => {
    setSavingsExpanded((prev) => !prev);
  };
  ...
  return (
    <div className="mt-3">
      ...
      >
        ...
        <span className="text-gray-400">{savingsExpanded ? '▼' : '▶'}</span>
      </button>
